...
function validOption(numberArray, row, col, numToAdd, recursion){
    ...
    var possibilites;
    try { possibilites = fillPossibilities(numberArray, recursion);
...
function fillPossibilities(numberArray, recursion){
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
            if (numberArray[i][j] === -1) {
                var temp = fillSquarePossibilities(numberArray, i, j, recursion);
...
function fillSquarePossibilities(numberArray, row, col, recursion){
    ...
    for (let i = 1; i <= n; i++){
       ...
       // console.log("row: ", row, " col: ", col);
        ...
        if (validOption(numberArray, row, col, i, recursion)){
